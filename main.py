        ...
        try:
            async with session.put(f"{JSONBIN_API_URL}/{JSONBIN_BIN_ID}", json=data, headers=headers, timeout=10) as r:
                if r.status == 200:
                    ...
                    log.error(f"Lỗi lưu bin: {r.status} - {await r.text()}")
        except Exception as e:
...
if __name__ == "__main__":
    ...
    # SỬA LỖI PYTHON 3.14: Dùng asyncio.run thay vì get_event_loop
    ...
    app_bot.run_polling(drop_pending_updates=True)request_log = defaultdict(list)
muted_users = {}
processing_users = set()
