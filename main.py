...
def save_data():
    try:
        requests.put(f"{JSONBIN_API_URL}/{JSONBIN_BIN_ID}",
                     json={"keys":keys,"users":users,"used_accounts":list(used_accounts)},
...
async def start(update, context):
    ...
    if uid == ADMIN_ID: await update.message.reply_text(admin_menu_text(), parse_mode="Markdown")
        ...
        await update.message.reply_text("🤖 *LQ ACC BOT*\n\n🔐 Chưa có key.\n👉 Nhấn nút dưới.", reply_markup=login_menu(), parse_mode="Markdown")
...
async def button_handler(update, context):
    ...
    if data.startswith("lay_"):
        ...
        t0 = time.time()
        # SỬA: Chạy trong executor để không block event loop
    ...
    elif data == "key_input": await query.edit_message_text("📝 /key <mã>")
...
async def key_cmd(update, context):
    ...
    if key in keys:
        ...
        if time.time() - kd.get("created_ts",0) > KEY_EXPIRE_DAYS*86400: return await update.message.reply_text("❌ Hết hạn")
...
async def getkey(update, context): await update.message.reply_text("🔑 t.me/chantuiii")
...
async def users_list(update, context):
    ...
    await update.message.reply_text("👥 *Users:*\n" + "\n".join(f"👤 {u.get('name',uid)} | `{uid}`" for uid,u in list(users.items())[:30] if uid not in ("admin",str(ADMIN_ID))), parse_mode="Markdown")

async def stats(update, context):
    if not is_admin(update.effective_user.id): return
    nk = sum(1 for v in keys.values() if not v.get("vip"))
    vk = sum(1 for v in keys.values() if v.get("vip"))
    await update.message.reply_text(f"📊 Key: {nk}+{vk}VIP | 👥 {len(users)} | 📦 {sum(u.get('used',0) for u in users.values())} | 📋 {len(used_accounts)} | 🤐 {len(muted_users)}")

async def listkeys(update, context):
    if not is_admin(update.effective_user.id): return
    if not keys: return await update.message.reply_text("📭 Trống")
...
async def muted_list(update, context):
    if not is_admin(update.effective_user.id): return
    if not muted_users: return await update.message.reply_text("📭 Không ai mute")
    now=time.time()
