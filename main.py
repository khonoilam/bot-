    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.put(f"{JSONBIN_API_URL}/{JSONBIN_BIN_ID}", json=data, headers=headers, timeout=10) as r:
                if r.status == 200: log.info("Đã lưu lên jsonbin.io")
                else: log.error(f"Lỗi lưu bin: {r.status}")
        except Exception as e: log.error(f"Lỗi kết nối khi lưu: {e}")
...
async def initialize_data():
    ...
    users = {}
    used_accounts = set()
    log.warning("Không tải được dữ liệu từ bin, khởi tạo rỗng.")
...
async def start(update, context):
    ...
    if str(uid) in users: users[str(uid)]["name"] = name; await safe_save()
...
async def button_handler(update, context):
    ...
    if check_spam(uid): await query.edit_message_text("🚫 Bạn đã bị cấm chat 60 giây vì spam!"); return
    if data.startswith("lay_"):
        ...
        if uid != ADMIN_ID:
            ...
            if remaining <= 0: await query.edit_message_text(f"🚫 Hết {limit} acc/ngày\n/key <mã>"); return
        ...
        t0 = time.time(); accs = await fetch_fast(n); t1 = time.time() - t0
        ...
        if uid != ADMIN_ID: await safe_save()
...
async def key_cmd(update, context):
    ...
    if not context.args: return await update.message.reply_text("🔐 Key không đúng! Nhấn nút Lấy Key Free để được cấp key.")
    ...
    if key in keys:
        ...
        del keys[key]; await safe_save()
        limit = LIMIT_VIP if is_vip else LIMIT_NORMAL
        kb, text = main_menu(int(uid))
        await update.message.reply_text(f"✅ {'👑 VIP' if is_vip else '🔑 Thường'} | {limit} acc/ngày", reply_markup=kb, parse_mode="Markdown")
    else: await update.message.reply_text("🔐 Key không đúng! Nhấn nút Lấy Key Free để được cấp key.")
...
async def lay(update, context):
    ...
    if check_spam(uid): await update.message.reply_text("🚫 Bạn đã bị cấm chat 60 giây vì spam!"); return
    u = get_user(uid)
    ...
    if uid != ADMIN_ID:
        ...
        if remaining <= 0: return await update.message.reply_text(f"🚫 Hết acc\n/key <mã>")
    processing_users.add(uid); msg = await update.message.reply_text(f"⏳ {n} acc...")
    accs = await fetch_fast(n); processing_users.discard(uid)
...
async def getkey(update, context): await update.message.reply_text("🔑 Nhấn nút *Lấy Key Free* để ib t.me/chantuiii nhận key miễn phí!\nSau khi có key: /key <mã>", parse_mode="Markdown")

async def genkey(update, context):
    if not is_admin(update.effective_user.id): return
    try: n = int(context.args[0]) if context.args else 1
    except: n = 1
    if n > 20: n = 20
    new_keys = []
    for _ in range(n):
...
async def key_status(update, context):
    ...
    if keys:
        text += "🔑 *Chưa dùng:*\n"
        for k, v in list(keys.items())[:15]: text += f"`{k}` ({'VIP' if v.get('vip') else 'TH'})\n"
    if users:
        text += "\n👥 *Đã dùng:*\n"
        for uid, u in list(users.items())[:20]:
            if uid in ("admin", str(ADMIN_ID)): continue
            name = u.get("name", uid); key = u.get("key","?")[:8]
            vip = "VIP" if u.get("vip") else "TH"; daily = u.get("daily_used",0)
            limit = get_limit(int(uid)); banned = "🚫" if u.get("banned") else ""
            text += f"👤 {name} | ID:`{uid}` | Key:{key}... ({vip}) | {daily}/{limit} {banned}\n"
    if not keys and not users: text += "📭 Chưa có dữ liệu"
...
async def listkeys(update, context):
    ...
    text = "🔑 *Kho key:*\n"
    for k, v in list(keys.items())[:20]: text += f"`{k}` ({'VIP' if v.get('vip') else 'TH'})\n"
    await update.message.reply_text(text, parse_mode="Markdown")
...
async def ban(update, context):
    ...
    uid = context.args[0]
    if uid in users: users[uid]["banned"] = True; await safe_save(); await update.message.reply_text(f"✅ Đã ban {uid}")
...
async def unban(update, context):
    ...
    uid = context.args[0]
    if uid in users: users[uid]["banned"] = False; await safe_save(); await update.message.reply_text(f"✅ Đã unban {uid}")
    else: await update.message.reply_text("❌ Không tìm thấy")

async def revoke(update, context):
    if not is_admin(update.effective_user.id): return
    if not context.args: return await update.message.reply_text("/revoke <id>")
    uid = context.args[0]
    if uid in users: del users[uid]; await safe_save(); await update.message.reply_text(f"✅ Đã thu hồi key của {uid}")
    else: await update.message.reply_text("❌ Không tìm thấy")
...
async def reset(update, context):
    ...
    if context.args:
        uid = context.args[0]
        if uid in users: users[uid]["daily_used"] = 0; users[uid]["date"] = today_vn(); await safe_save(); await update.message.reply_text(f"✅ Đã reset {uid}")
        else: await update.message.reply_text("❌ Không tìm thấy")
    else:
        for u in users: users[u]["daily_used"] = 0; users[u]["date"] = today_vn()
        await safe_save(); await update.message.reply_text("✅ Đã reset tất cả")

async def delkey(update, context):
    if not is_admin(update.effective_user.id): return
    if not context.args: return await update.message.reply_text("/delkey <key>")
    key = context.args[0].strip().upper()
    if key in keys: del keys[key]; await safe_save(); await update.message.reply_text("✅ Đã xóa key")
    else: await update.message.reply_text("❌ Key không tồn tại hoặc đã dùng")
...
async def unmute_cmd(update, context):
    if not is_admin(update.effective_user.id): return
    if not context.args: return await update.message.reply_text("/unmute <id>")
    uid = int(context.args[0])
    if uid in muted_users: del muted_users[uid]; request_log[uid].clear(); await update.message.reply_text(f"✅ Đã bỏ mute {uid}")
    else: await update.message.reply_text(f"❌ {uid} không bị mute")

async def resetall(update, context):
    if not is_admin(update.effective_user.id): return
    if not context.args or context.args[0] != "CONFIRM": return await update.message.reply_text("⚠️ Xác nhận: /resetall CONFIRM")
    keys.clear(); users.clear(); used_accounts.clear(); request_log.clear(); muted_users.clear(); processing_users.clear()
    await safe_save(); log.warning("RESET ALL!"); await update.message.reply_text("☢️ Đã xóa TOÀN BỘ dữ liệu!")
