...
CHECK_API_URL = "http://103.77.246.176:5000/check"
...
HEADERS = {
    ...
    "referer": "https://tangacc.net/", "user-agent": "Mozilla/5.0"
}
...
def _clean_expired_users_nolock():
    now = time.time()
    expired = [uid for uid, u in users.items() if u.get("expire_ts") and now > u["expire_ts"]]
    for uid in expired: del users[uid]
    if expired: log.info(f"Đã xóa {len(expired)} user hết hạn")

def _get_user_copy(uid):
    if uid == ADMIN_ID:
        if "admin" not in users:
            users["admin"] = {"history":[],"used":0,"daily_used":0,"banned":False,"vip":True,"last_acc":[],"checked_acc":[],"expire_ts":0}
        return copy.deepcopy(users["admin"])
    u = users.get(str(uid))
    if u:
        if u.get("expire_ts") and time.time() > u["expire_ts"]:
            del users[str(uid)]
            save_data()
            return {}
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                with users_lock:
                    ...
                    _clean_expired_users_nolock()
                with acc_lock: used_accounts = set(data.get("used_accounts", []))
    ...
    with data_lock:
        for attempt in range(2):
            try:
                ...
                with acc_lock: a = list(used_accounts)
...
async def check_acc_new(session, username, password):
    for retry in range(3):
        try:
            async with session.post(CHECK_API_URL, data={"user":username,"pass":password},
                                    timeout=aiohttp.ClientTimeout(total=CHECK_TIMEOUT)) as resp:
                if resp.status != 200: return {"status":"ERROR","username":username}
                d = await resp.json()
                ...
                else: return {"status":"MISS","username":username}
...
async def start(update, context):
    ...
    with users_lock:
        if str(uid) in users:
            u = users[str(uid)]
            if u.get("expire_ts") and time.time() > u["expire_ts"]:
                del users[str(uid)]; need_save = True; auth = False
            else:
        ...
        async def fetch_and_send():
            try:
                ...
                with users_lock:
                    ...
                    u["history"] = (u.get("history",[]) + accs)[-MAX_HISTORY:]
                    u["used"] = u.get("used",0) + len(accs); u["daily_used"] = u.get("daily_used",0) + len(accs)
                    u["last_acc"] = accs.copy(); last_copy = list(accs); total_acc = len(last_copy)
        ...
        with users_lock:
            ...
            if uid == ADMIN_ID: limit=99999; check_rem=99999; expire_str="Vĩnh viễn"
                ...
                if u.get("expire_ts"):
                    ...
                    expire_str = f"{int(rs//3600)}h{int((rs%3600)//60)}m" if rs > 0 else "Hết hạn"
...
async def key_cmd(update, context):
    ...
    with users_lock:
        ...
        users[uid] = {"key":key,"vip":is_vip,"used":0,"daily_used":0,"date":today_vn(),"banned":False,"history":[],
                      ...
                      "last_acc":[],"checked_acc":[],"expire_ts":time.time()+KEY_EXPIRE_SECONDS}
    await save_data()
...
async def lay(update, context):
    ...
    if uid in active_tasks and not active_tasks[uid].done():
        await update.message.reply_text("⏳ Đang lấy acc, vui lòng đợi."); return
...
async def genkey(update, context):
    ...
    with keys_lock:
        ...
        for _ in range(n):
            ...
            keys[k] = {"type":"normal","created_ts":time.time(),"banned":False,"vip":False}
            new.append(k)
    await save_data()
...
async def genvip(update, context):
    ...
    with keys_lock:
        ...
        for _ in range(n):
            k = secrets.token_hex(8).upper()
            keys[k] = {"type":"vip","vip":True,"created_ts":time.time(),"banned":False}
            new.append(k)
    await save_data()
...
async def key_status(update, context):
    ...
    with users_lock:
        u_list = list(users.items())[:20]; u_text = ""
        for uid, u in u_list:
